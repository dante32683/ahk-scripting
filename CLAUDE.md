...
7. **Local Extension Hooks** — optional `custom\Core_custom.ahk`, `custom\Master_custom.ahk`, and `custom\Master-PC_custom.ahk` files hold private machine-specific behavior outside the tracked repo.
8. **Window Management Helpers** — `_ApplyLayout` is the single tiling primitive. All tiling functions call it.
9. **Hyper Layer** — `#HotIf GetKeyState("CapsLock", "P")` block; CapsLock acts as a modifier. Desktop/monitor switching is machine-specific and lives in the entry points.
10. **Keyboard Lock** — `CapsLock+Alt+L` toggles `BlockInput`. Unlock by typing `"unlock"` on the physical keyboard.
11. **App Launchers** — `_ActivateOrRunOnCurrentDesktop` ensures hotkey-launched apps stay on the current virtual desktop.
12. **Camera Toggle** — Copilot key (`#+F23`) toggles device via `pnputil.exe` (with PowerShell fallback).
