...
4. **Desktop Focus Memory** — `DesktopLastWindow` Map holds last-focused HWND per desktop. Written to `%TEMP%\ahk_desktop_memory.ini` only on exit via `OnExit`.
5. **Window Management Helpers** — `_ApplyLayout(x%, y%, w%, h%)` is the single tiling primitive. All named tile functions (`TileLeft`, `TileRight`, `TileTopLeft`, etc.) are one-liners calling it. Uses DWM API (`DwmGetWindowAttribute`) for invisible border compensation.
6. **Hyper Layer** — `#HotIf GetKeyState("CapsLock", "P")` block; CapsLock acts as a modifier. WASD = arrows, HJKL = focus direction, Z/X/number keys = tiling/desktops.
7. **Camera Toggle** — Copilot key (`#+F23`) uses WMI (pre-initialized at startup) to query device state, then `pnputil.exe` (with PowerShell fallback) to enable/disable by `CFG_CameraID`.
8. **Workspace Launcher** (`Win+Ctrl+S`) — `WorkspaceLayout()` returns an array of `{Type, Launch, Match, Desktop}` objects. The launcher iterates them, opens missing apps, and moves each to its target desktop via VDA.
...
- **Tiling** — extend by adding a new one-liner calling `_ApplyLayout`. Adjust `TileGap` / `TileBorder` globals for spacing.
- **Workspace layout** — edit `WorkspaceLayout()` in `Master.ahk` to add/remove/reorder apps. Supported types: `"browser"` (detected by new HWND), `"pwa"` (detected by title), `"app"` (detected by title or exe).
