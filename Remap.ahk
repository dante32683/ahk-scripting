...
!`:: _CycleSameAppWindow(1)
!+`:: _CycleSameAppWindow(-1)

#HotIf

_CycleSameAppWindow(dir) {
    if !WinExist("A")
        return
    curHwnd := WinGetID("A")
...
_IsGameActive() {
    ...
    try {
        ...
        lastHwnd := hwnd
        procPath := WinGetProcessPath("ahk_id " hwnd)
        procName := WinGetProcessName("ahk_id " hwnd)

        isGame := false
        if IsSet(CFG_GameProcesses) && CFG_GameProcesses {
            for game in CFG_GameProcesses {
                if (StrLower(game) = StrLower(procName)) {
                    isGame := true
                    break
                }
            }
        ...
        if !isGame {
            pathLower := StrLower(procPath)
            if InStr(pathLower, "\steamapps\")
                || InStr(pathLower, "\steamlibrary\")
                || InStr(pathLower, "\epic games\")
                || InStr(pathLower, "\gog games\")
                || InStr(pathLower, "\gog galaxy\")
                || InStr(pathLower, "\origin games\")
                || InStr(pathLower, "\ea games\")
                || InStr(pathLower, "\ubisoft\")
                || InStr(pathLower, "\riot games\")
                || InStr(pathLower, "\xboxgames\")
                || InStr(pathLower, "\blizzard\")
                || InStr(pathLower, "\battle.net\") {
                isGame := true
            }
        }
